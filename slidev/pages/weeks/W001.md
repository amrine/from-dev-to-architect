...
TeamPulse doit permettre à un administrateur plateforme de créer une **organisation**, puis à son administrateur de construire les **équipes**, les **appartenances** et les **rôles**.
...
Installez la scène : trois développeurs prennent en charge organisation, identité et équipe. Le premier parcours visé sépare déjà le compte plateforme, l'utilisateur d'organisation et l'appartenance à une équipe. Chacun peut réussir isolément, mais vendredi leurs contributions devront démarrer et être testées ensemble.
...
```mermaid {scale: 0.62}
flowchart LR
  P["Admin plateforme<br/>crée l'organisation"] --> O["Admin organisation<br/>crée l'équipe"]
  O --> U["Affecter un utilisateur<br/>avec un rôle"]
  U --> C["Ouvrir une campagne<br/>plus tard"]
```
...
- Un compte plateforme reste hors de toutes les organisations
- Un utilisateur métier appartient à une organisation et à plusieurs équipes possibles
...
Commencez par le flux plateforme, organisation, équipe, puis campagne. Séparez clairement les deux horizons : cette semaine, l'équipe doit travailler et démarrer de façon cohérente ; les tickets T04 et T05 documentent la frontière des comptes, les rôles et l'appartenance temporelle qui porteront la première API d'administration.
...
| Isoler plateforme et tenant | Un compte plateforme ne reçoit aucun accès aux données d'une organisation |
| Contextualiser les rôles | Une permission dépend de l'organisation, de l'équipe et de l'appartenance active |
| Préserver l'historique | Retirer un membre clôture une période sans supprimer ses contributions |
| Détecter tôt une erreur | Une dépendance interdite, un SQL invalide ou un accès cross-org fait échouer le test |
...
Lisez chaque ligne de gauche à droite. Par exemple, « partir du même état » devient « deux postes démarrent le même PostgreSQL avec une commande ». Puis rattachez les nouvelles garanties au scénario : le compte plateforme reste dehors, une permission porte un périmètre et un retrait conserve l'histoire. Ces formulations permettent déjà d'imaginer une preuve.
...
- Un compte plateforme distinct des utilisateurs d'organisation
- Une appartenance d'équipe temporelle et des rôles contextualisés
...
- Livrer déjà campagnes, résultats, propositions et votes
...
Présentez d'abord les contraintes retenues : une unité déployable, une base, plusieurs domaines, une frontière plateforme/tenant, des appartenances temporelles et des tests autonomes. Puis montrez ce qui est volontairement reporté. Le but est de résoudre le problème d'intégration et de préparer la première administration sans introduire réseau, cloud ou campagne hebdomadaire avant qu'ils soient nécessaires.
...
<div class="tp-card">
<h3>La plateforme reste hors tenant</h3>
<p class="small muted">Le compte plateforme crée l'organisation sans pouvoir appeler ses ports métier.</p>
</div>

<div class="tp-card">
<h3>Un retrait conserve la période</h3>
<p class="small muted"><code>endedAt</code> révoque l'accès sans supprimer l'appartenance ni ses références historiques.</p>
</div>
...
[click] Les deux dernières preuves annoncent T04 et T05 : séparation plateforme/tenant, puis appartenance temporelle lors d'un retrait.
...
| `T04` | Comment séparer compte plateforme, tenant et données d'organisation ? | <span class="tp-badge tp-badge--doc">documenté</span> |
| `T05` | Comment porter rôles, appartenances temporelles et première API d'administration ? | <span class="tp-badge tp-badge--doc">documenté</span> |
...
Présentez T01, T02 et T03 comme trois fondations déjà livrées : frontières entre modules, donnée reproductible, puis règles internes exécutables. T04 et T05 sont désormais documentés : ils séparent les comptes, contextualisent les rôles et préparent l'API d'administration. T06 et T07 restent planifiés pour l'image et les environnements.
...
flowchart LR
  ...
  T03 --> T04["T04<br/>Comptes · tenant"]
  T04 --> T05["T05<br/>Rôles · appartenance"]
...
- On sépare le compte plateforme de l'utilisateur d'organisation avant d'attribuer des rôles
- On modélise une appartenance temporelle avant d'exposer le retrait par API
...
Lisez la chaîne de gauche à droite. On définit d'abord les responsabilités, puis l'espace de données de chacune. On verrouille la direction des dépendances avant d'introduire la frontière plateforme/tenant. Les rôles et les périodes d'appartenance s'appuient sur cette frontière, puis l'image Docker vient après la validation du comportement et les environnements s'appuient sur cette image.
...
<br/><span class="muted">T04 distingue plateforme et tenant ; T05 ajoute rôles contextualisés et appartenance temporelle sans déplacer JPA dans <code>tp-common</code>.</span>
</div>
...
Lisez strictement de haut en bas. `tp-app` démarre le runtime. Les quatre modules organisent les responsabilités. Dans chaque module métier, les détails techniques devront dépendre du cœur, jamais l'inverse. Le `DataSource` contient les paramètres d'une connexion PostgreSQL. Les schémas organisent les tables dans cette même base. T04 et T05 prolongeront ce socle sans ajouter de classe JPA à `tp-common`.
...
flowchart TB
  ...
  TC["tp-test-support<br/>PostgreSQLTestConfiguration"] -. remplace la connexion .-> DS
...
<p><strong>5.</strong> La configuration Testcontainers est mutualisée dans <code>tp-test-support</code>, au scope Maven <code>test</code>.</p>
...
[click] `tp-test-support` fournit la configuration Testcontainers uniquement aux classpaths de test ; elle n'est ni chargée ni empaquetée en production.
...
## Annexe T02 · Testcontainers est partagé par `tp-test-support`
...
<div class="tp-card tp-card--pulse mt-3 small">
La configuration vit dans <code>tp-test-support</code>, dépendance Maven limitée au scope <code>test</code>. L'import explicite ne crée aucune dépendance dans le code de production.
</div>
...
L'infrastructure PostgreSQL de test est mutualisée dans un module technique réservé aux tests, puis importée explicitement par chaque scénario qui en a besoin.

**Déroulé oral**

Commencez par `@TestConfiguration` : Spring identifie une configuration réservée aux tests. `TeamPulsePostgreSQLContainer` centralise l'image et les paramètres PostgreSQL, tandis que `@ServiceConnection` transmet automatiquement la connexion au `DataSource`.

Montrez ensuite le test de module. `@Import` rend le besoin de PostgreSQL explicite et la dépendance Maven vers `tp-test-support` reste limitée au scope `test`.

[click] Le test global réutilise la même infrastructure, mais assemble les trois configurations Flyway.

**Insister sur**

Mutualiser le conteneur ne transforme pas `tp-test-support` en module métier ou en dépendance du runtime. Le code de production ne peut pas l'utiliser.
